...
class CLIENT: 
  ...
  def client(self, data, user=None, name=None):
     # A unique in-memory client name prevents collisions when different
     # forwarding jobs use different saved accounts at the same time.
     client_name = name or ("USERBOT" if user is True or (user is None and isinstance(data, dict) and not data.get('is_bot')) else "BOT")
     if user is None and isinstance(data, dict) and data.get('is_bot') == False:
        return Client(client_name, self.api_id, self.api_hash, session_string=data.get('session'))
     elif user is True:
        return Client(client_name, self.api_id, self.api_hash, session_string=data)
     elif user != False:
        data = data.get('token')
     return Client(client_name, self.api_id, self.api_hash, bot_token=data, in_memory=True)
