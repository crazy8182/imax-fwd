  ...
  if type=="main":
     ...
     for account in accounts:
        ...
        buttons.append([InlineKeyboardButton(
            f"{icon} {account['name']} ({username})",
            callback_data=f"settings#editbot_{account['id']}"
        )])
     ...
     await query.message.edit_text(
       f"<b><u>MY BOTS & USERBOTS</u></b>\n\n<b>You currently have {len(accounts)} account(s) added.</b>\n\n"
       "<i>Add as many bots and userbots as you like. You can choose the account when starting a forwarding job.</i>",
     ...
     await bot.send_message(
        user_id,
        f"<b>🤖 Bot successfully added.</b>\n\n<b>Total accounts: {len(accounts)}</b>",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('🤖 My Bots & Userbots', callback_data='settings#bots')]])
     )
     ...
     await bot.send_message(
        user_id,
        f"<b>👤 Userbot successfully added.</b>\n\n<b>Total accounts: {len(accounts)}</b>",
        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton('🤖 My Bots & Userbots', callback_data='settings#bots')]])
     )
     ...
     await query.answer()
     await query.message.delete()
     result = await CLIENT.add_login(bot, query)
     if not result:
        return
     accounts = await db.get_bots(user_id)
     await bot.send_message(
     ...
     account = await db.get_bot_by_id(user_id, int(bot_id))
     if not account:
        return await query.answer("Account not found", show_alert=True)
     TEXT = Translation.BOT_DETAILS if account['is_bot'] else Translation.USER_DETAILS
     buttons = [[InlineKeyboardButton('❌ Remove This Account ❌', callback_data=f"settings#removebot_{account['id']}")],
                [InlineKeyboardButton('↩ Back', callback_data="settings#bots")]]
     await query.message.edit_text(
        TEXT.format(account['name'], account['id'], account['username'] or 'private'),
        reply_markup=InlineKeyboardMarkup(buttons))
                                             
  elif type.startswith("removebot_"):
     bot_id = type.split('_', 1)[1]
     await db.remove_bot(user_id, int(bot_id))
     accounts = await db.get_bots(user_id)
     buttons = []
     for account in accounts:
        icon = "🤖" if account.get('is_bot') else "👤"
     ...
     buttons += [[InlineKeyboardButton('✚ Add Bot', callback_data="settings#addbot")],
                 [InlineKeyboardButton('✚ Add User Bot', callback_data="settings#adduserbot")],
                 [InlineKeyboardButton('🔐 Login User Bot', callback_data="settings#addlogin")],
